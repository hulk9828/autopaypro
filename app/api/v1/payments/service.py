...
class PaymentService:
    ...
    async def record_external_payment(
        ...
        """
        Record a payment from an external system. No auth. Validates customer, loan, ownership;
        applies amount to earliest unpaid installments; creates payment with payment_method='external';
        updates loan total_paid. Uses database transaction.
        Returns dict: success (bool), message (str), and on success: payment_id, remaining_balance.
        On validation error returns success=False, message=error.
        """
        if amount <= 0:
        ...
        if loan.customer_id != customer_id:
            return {"success": False, "message": "Loan does not belong to this customer"}
        if getattr(loan, "status", "active") == "closed":
            return {"success": False, "message": "Loan already completed"}
        remaining_before = _round_amount(
            float(loan.amount_financed) - float(getattr(loan, "total_paid", 0))
        )
        if remaining_before <= 0:
            return {"success": False, "message": "Loan already completed"}
        apply_amount = _round_amount(min(amount, remaining_before))
        if apply_amount <= 0:
            return {"success": False, "message": "Invalid amount"}
        ...
        loan.total_paid = _round_amount(float(getattr(loan, "total_paid", 0)) + apply_amount)
        loan.amount_financed = _round_amount(max(0, float(loan.amount_financed) - apply_amount))
    ...
    async def create_checkout_admin(
        self,
        customer_id: UUID,
        loan_id: UUID,
        ...
        remaining = _round_amount(
            float(loan.amount_financed) - float(getattr(loan, "total_paid", 0))
        )
        if remaining <= 0:
            AppException().raise_400("Loan already completed")
        apply_amount = _round_amount(amount) if amount is not None and amount > 0 else remaining
        if apply_amount <= 0:
    ...
    async def get_checkout_by_token(self, token: str) -> dict | None:
        """
        Fetch checkout by token (no auth). Returns dict for GetCheckoutResponse or None if not found.
        If checkout is expired, updates status to expired and returns None.
        """
        result = await self.db.execute(
            select(Checkout, Customer, Loan, Vehicle)
            .join(Customer, Checkout.customer_id == Customer.id)
            .join(Loan, Checkout.loan_id == Loan.id)
            .join(Vehicle, Loan.vehicle_id == Vehicle.id)
            .where(Checkout.token == token)
        )
        row = result.first()
        if not row:
            return None
        checkout, customer, loan, vehicle = row
        if checkout.status != "pending":
            return {
                ...
                "remaining_balance": _round_amount(float(loan.amount_financed) - float(getattr(loan, "total_paid", 0))),
        ...
        return {
            ...
            "loan_id": checkout.loan_id,
            "vehicle_display": f"{vehicle.year} {vehicle.make} {vehicle.model}" if vehicle else None,
            "amount": float(checkout.amount or remaining_balance),
            "remaining_balance": remaining_balance,
            "status": checkout.status,
            "expires_at": checkout.expires_at,
        }

    async def complete_checkout(self, token: str, amount: float | None = None) -> dict:
        """
        Complete checkout: validate token (pending, not expired), then record payment via record_external_payment,
        mark checkout as completed. Returns success, message, payment_id, remaining_balance or error dict.
        """
        result = await self.db.execute(
            select(Checkout, Customer, Loan)
            .join(Customer, Checkout.customer_id == Customer.id)
            .join(Loan, Checkout.loan_id == Loan.id)
            .where(Checkout.token == token)
        )
        ...
        checkout, customer, loan = row
        if checkout.status != "pending":
            return {"success": False, "message": f"Checkout already {checkout.status}"}
        if checkout.expires_at < datetime.utcnow():
            checkout.status = "expired"
            self.db.add(checkout)
            await self.db.commit()
            return {"success": False, "message": "Checkout link has expired"}
        pay_amount = amount if amount is not None and amount > 0 else (checkout.amount or 0)
        if pay_amount <= 0:
            remaining = _round_amount(float(loan.amount_financed) - float(getattr(loan, "total_paid", 0)))
            pay_amount = remaining
        if pay_amount <= 0:
            return {"success": False, "message": "Loan already paid"}
        payment_result = await self.record_external_payment(
